...
<div id="zan-page" class="clearfix">
	<ul class="pagination pagination-zan">
	  <?php
		  echo "<li>";
...
//编辑或修改日志或页面时添加自定义字段
add_action('edit_page', 'add_custom_field_automatically');
add_action('edit_post', 'add_custom_field_automatically');
//为每篇新建日志或新建页面添加自定义字段
add_action('save_page', 'add_custom_field_automatically');
add_action('save_post', 'add_custom_field_automatically');
function add_custom_field_automatically($post_ID) {
    global $wpdb;
    if(!wp_is_post_revision($post_ID)) {
        add_post_meta($post_ID, 'amazon_shopping_url', '', true);
        add_post_meta($post_ID, 'amazon_price', '', true);
        add_post_meta($post_ID, 'jingdong_shopping_url', '', true);
        add_post_meta($post_ID, 'jingdong_price', '', true);
        add_post_meta($post_ID, 'guanfang_shopping_url', '', true);
        add_post_meta($post_ID, 'guanfang_price', '', true);        
        add_post_meta($post_ID, 'taobao_shopping_url', '', true);
        add_post_meta($post_ID, 'taobao_price', '', true);        
    }
}
